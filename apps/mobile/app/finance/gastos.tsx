	...
	const loadData = useCallback(async () => {
		...
		try {
			...
			const { data } = await supabase
				.from("gastos")
				.select("descricao, valor, data, classificacao")
	...
	return (
		<SafeAreaView style={tw`flex-1 bg-gray-50`}>
			...
			<ScrollView horizontal>
				<View>
					<View style={tw`flex-row bg-red-700 p-3`}>
						...
						{loading ? (
							...
							dataMatrix.map((row, i) => (
								<View
									...
									<Text style={tw`w-24 text-center font-bold text-red-700`}>
			...
			<TouchableOpacity
				...
				style={tw`absolute bottom-8 right-8 bg-red-700 w-16 h-16 rounded-full justify-center items-center shadow-lg`}>
				<Text style={tw`text-white text-3xl`}>+</Text>
			</TouchableOpacity>

			{/* Modal estendido com categorias para Gastos */}
			<FinanceModal
				...
				onSave={handleSave}>
				<View style={tw`mt-4`}>
					<Text style={tw`text-gray-400 text-xs mb-2 uppercase font-bold`}>
						Classificação
					</Text>
					<ScrollView horizontal showsHorizontalScrollIndicator={false}>
						{CATEGORIAS.map((c) => (
							<TouchableOpacity
								...
								style={tw`bg-gray-100 px-3 py-2 rounded-lg mr-2 border ${classificacao === c ? "border-red-600" : "border-transparent"}`}>
								<Text
									style={tw`${classificacao === c ? "text-red-700 font-bold" : "text-gray-600"}`}>
									{c}
								</Text>
							</TouchableOpacity>
						))}
